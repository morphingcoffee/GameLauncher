...
The app is the single `:composeApp` desktop module. All Gradle commands run from `launcher/` (wrapper is Gradle 8.14.2). Standard commands live in [`README.md`](README.md) and [`launcher/composeApp/build.gradle.kts`](launcher/composeApp/build.gradle.kts); the essentials are:
...
- **JDK 21 is the toolchain here** (project compiles to bytecode 17 via `jvmTarget`/`sourceCompatibility`). Lint, tests, and run all work on JDK 21 — no JDK 17 install is needed.
- **Android SDK is required for Gradle sync**, even though the shipped app is desktop-only: every module declares an `androidTarget` library stub (for Compose `@Preview`). The SDK lives at `~/android-sdk` (platform 35 + build-tools 35). Gradle finds it via `launcher/local.properties` (`sdk.dir=…`, gitignored) — the startup update script recreates this file if missing. If Gradle errors with "SDK location not found", ensure that line exists or export `ANDROID_HOME`.
