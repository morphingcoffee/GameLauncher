...
**Publish to R2** is a separate workflow: **Actions → Publish launcher release** — pass the **run ID** from a successful Desktop installers run. By default **Require all four prod channels** is on (full cross-platform release); turn it off to publish only the prod artifacts present in that build run. The workflow uploads blobs, commits `manifests/manifest.json`, and publishes to R2.
